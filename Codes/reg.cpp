...
int main() {
    ...
    resultfilepolyhedron.open("polyr.csv");
    ...
    if (file1.is_open() && resultfileedge.is_open()) {
        while (getline(file1, line1)) {
            if (line1.substr(0,6).compare("Line (")) continue;
            sregex_iterator current1(line1.begin(), line1.end(), reg);
    ...
    if (file2.is_open() && resultfilefacet.is_open()) {
        while (getline(file2, line2)) {
            if (line2.substr(0,9).compare("Line Loop")) continue;
    ...
    if (file3.is_open() && resultfilepolyhedron.is_open()) {
        while (getline(file3, line3)) {
            if (line3.substr(0,12).compare("Surface Loop")) continue;
